...
$_RAASForm_FormTab = function (FormTab $formTab) use (
      ...
      <table class="table table-striped table-condensed" data-role="raas-repo-block">
        <tbody data-role="raas-repo-container">
          <?php foreach ((array)$DATA['access_id'] as $i => $temp) { ?>
              <tr data-role="raas-repo-element">
                ...
                <td>
                  <div data-role="access-uid" <?php echo $DATA['access_to_type'][$i] == CMSAccess::TO_USER ? '' : 'style="display: none"'?>>
                    <?php
                    $u = new User($DATA['access_uid'][$i]);
                    $name = $u->login;
                    if ($u->full_name) {
                  ...
                  <div data-role="access-gid" <?php echo $DATA['access_to_type'][$i] == CMSAccess::TO_GROUP ? '' : 'style="display: none"'?>>
                    <select name="access_gid[]">
                        <?php echo $showGroups(new Group(), $DATA['access_gid'][$i])?>
                    </select>
                  </div>
                </td>
                <td>
        ...
        <tbody>
          <tr data-role="raas-repo">
            ...
            <td>
              <div data-role="access-uid" style="display: none">
                <input type="hidden" name="access_uid[]" disabled="disabled" />
              </div>
              <div data-role="access-gid" style="display: none">
                <select name="access_gid[]">
                  <?php echo $showGroups(new Group())?>
                </select>
              </div>
            </td>
            <td>
    ...
    <script src="<?php echo Application::i()->view->context->publicURL?>/cmsaccess.js"></script>
<?php } ?>
