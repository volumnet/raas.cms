      ...
      <table class="table table-striped table-condensed" data-role="raas-repo-block">
        ...
        <tbody data-role="raas-repo-container">
          <?php foreach ((array)$DATA['filter_var'] as $i => $temp) { ?>
              <tr data-role="raas-repo-element">
                <td><input type="text" name="filter_var[]" value="<?php echo htmlspecialchars($DATA['filter_var'][$i])?>" class="span2" /></td>
                <td>
                  <select name="filter_relation[]" class="span3">
                    <?php foreach (\RAAS\CMS\Block_Material::$filterRelations as $key => $val) { ?>
        ...
        <tbody>
          <tr data-role="raas-repo">
            <td><input type="text" name="filter_var[]" value="" class="span3" disabled="disabled" /></td>
            <td>
              <select name="filter_relation[]" class="span2" disabled="disabled">
                <?php foreach (\RAAS\CMS\Block_Material::$filterRelations as $key => $val) { ?>
                    <option value="<?php echo htmlspecialchars($key)?>"><?php echo constant('CMS\\' . $val)?></option>
                <?php } ?>
              </select>
            </td>
            <td>
              <select name="filter_field[]" class="jsMaterialTypeField span2" disabled="disabled">
                <?php foreach ($CONTENT['fields'] as $row) { ?>
                    <option value="<?php echo htmlspecialchars($row['value'])?>"><?php echo htmlspecialchars($row['caption'])?></option>
                <?php } ?>
              </select>
            </td>
            <td><a href="#" class="close" data-role="raas-repo-del">&times;</a></td>
