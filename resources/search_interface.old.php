...
if (!$search_string) {
    ...
    $SQL_query = "SELECT SQL_CALC_FOUND_ROWS page_id AS id,
                    ...
                    FROM (
                       ...
                       LEFT JOIN " . Field::_tablename() . " AS tF ON tF.classname = 'RAAS\\CMS\\Material_Type' AND tF.datatype IN ('text', 'tel', 'email', 'url', 'textarea', 'htmlarea')
                       LEFT JOIN " . Field::_dbprefix() . "cms_data AS tD ON tD.pid = tM.id AND tD.fid = tF.id 
                           WHERE tP.vis 
                             AND NOT tP.response_code 
                             AND tM.vis 
                             AND (tM.name LIKE " . $SQL_search_string . " OR tM.description LIKE " . $SQL_search_string . " OR tD.value LIKE " . $SQL_search_string . ") 
                             ";
