    ...
    if (!$config['material_types'] || in_array(0, (array)$config['material_types'])) {
        $SQL_query .= "   UNION ALL 
                          ...
                          SELECT tP.id AS page_id, 
                                 ...
                                 0 AS rMname, 
                                 MAX((tBP.description LIKE " . $SQL_search_string . ") OR (tBH.description LIKE " . $SQL_search_string . ")) AS rMdescription, 
                                 0 AS rMdata
                       ...
                       LEFT JOIN " . Field::_dbprefix() . "cms_data AS tD ON tD.pid = tP.id AND tD.fid = tF.id
                            JOIN " . Block::_dbprefix() . "cms_blocks_pages_assoc AS tBPA ON tBPA.page_id = tP.id
                            JOIN " . Block::_tablename() . " AS tB ON tB.id = tBPA.block_id AND tB.vis AND tB.block_type IN ('html', 'php')
                       LEFT JOIN " . Block::_dbprefix() . "cms_blocks_html AS tBH ON tBH.id = tB.id
                       LEFT JOIN " . Block::_dbprefix() . "cms_blocks_php AS tBP ON tBP.id = tB.id
                           WHERE tP.vis AND (tP.name LIKE " . $SQL_search_string . " OR tD.value LIKE " . $SQL_search_string . " OR tB.description LIKE " . $SQL_search_string . ")
                             ";
    if ((array)$config['pages']) {
        $SQL_query .= " AND tP.id IN (" . implode(", ", array_map('intval', (array)$config['pages'])) . ") 
                        ";
