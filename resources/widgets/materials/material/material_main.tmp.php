<?php
/**
 * {{MATERIAL_TYPE_NAME}} для главной страницы
...
if ($Set) { ?>
    <div class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main">
      <div class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main__title h2">
        <<?php echo $nat ? 'a href="/{{MATERIAL_TYPE_CSS_CLASSNAME}}/"' : 'span'?>>
          <?php echo htmlspecialchars($Block->name)?>
        </<?php echo $nat ? 'a' : 'span'?>>
      </div>
      <div class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main__list {{MATERIAL_TYPE_CSS_CLASSNAME}}-main-list">
        <?php foreach ($Set as $item) { ?>
            <<?php echo ($nat ? 'a href="' . htmlspecialchars($item->url) . '"' : 'span')?>
              class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main-list__item {{MATERIAL_TYPE_CSS_CLASSNAME}}-main-item"
            >
              <img
                class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main-item__image"
                loading="lazy"
              ...
              <div class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main-item__text">
                ...
                <?php } ?>
                <div
                  class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main-item__title"
                  <?php echo $nat ? 'href="' . htmlspecialchars($item->url) . '"' : ''?>
                >
                  <?php echo htmlspecialchars($item->name)?>
                </div>
                <div class="{{MATERIAL_TYPE_CSS_CLASSNAME}}-main-item__description">
                  <?php
                  echo htmlspecialchars(Text::cuttext(
                      $item->brief ?:
                      html_entity_decode(strip_tags($item->description), ENT_COMPAT | ENT_HTML5, 'UTF-8'),
                      128,
                      '...'
                  ))?>
                </div>
