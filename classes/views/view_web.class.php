...
class View_Web extends \RAAS\Package_View_Web
{
    ...
    public function show_page(array $IN = array())
    {
        ...
        if ($IN['Item']->id) {
            ...
            $columns[' '] = array('callback' => function ($row, $i) use ($view, $IN) { return rowContextMenu($view->getPageContextMenu($row, $i, count($IN['Set']))); });
        } else {
            $columns['name'] = array(
                'caption' => $this->_('NAME'),
            ...
            $columns[' '] = array('callback' => function ($row) use ($view) { return rowContextMenu($view->getPageContextMenu($row)); });
        }
        $IN['Table'] = new Table(array('columns' => $columns, 'Set' => $IN['Set'], 'Pages' => $IN['Pages']));
        if ($IN['Item']->id) {
            $IN['Table']->class = 'table-condensed';
        } else {
            $IN['Table']->sort = $IN['sort'];
            $IN['Table']->order = ((strtolower($IN['order']) == 'desc') ? Column::SORT_DESC : Column::SORT_ASC);
            $IN['Table']->emptyString = $this->_('NO_SITES_FOUND');
        }
