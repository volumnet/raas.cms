...
class Block_HTML extends Block
{
    ...
    {
        if (!$this->name) {
            $this->name = htmlspecialchars($this->name ? $this->name : trim(\SOME\Text::cuttext(html_entity_decode(strip_tags($this->description), ENT_QUOTES, mb_internal_encoding()), 32, '...')));
        }
        parent::commit();
    }
